...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        overButton.frame = frame;
        overButton.backgroundColor = [UIColor clearColor];
        [overButton addTarget:self action:@selector(dismiss) forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:overButton];
        
        pickView = [[UIView alloc] init];
        pickView.backgroundColor = [UIColor colorWithRed:217.0/255.0 green:217.0/255.0 blue:217.0/255.0 alpha:1.0];
        pickView.frame = CGRectMake(0, frame.size.height, frame.size.width, PickViewHeight+ToolBarHeight);
        ...
        for (int i=0; i<UUPICKER_MINUTE; i++) {
            ...
            if (0<i && i<=UUPICKER_MONTH){
                [monthArray addObject:[num stringByAppendingString:@"月"]];
            }
            
            if (i<UUPICKER_HOUR){
                [hourArray addObject:[num stringByAppendingString:@"时"]];
            }
            
            [minuteArray addObject:[num stringByAppendingString:@"分"]];
        }
        for (int i=UUPICKER_MINDATE; i<UUPICKER_MAXDATE; i++) {
            NSString *num = [NSString stringWithFormat:@"%d",i];
            [yearArray addObject:[num stringByAppendingString:@"年"]];
        }
...
//            self.minLimitDate = [self dateFromString:@"197001010000" withFormat:@"yyyyMMddHHmm"];
            minDateModel = [[UUDatePicker_DateModel alloc]initWithDate:[NSDate date]];
        }
        ...
        UIToolbar *pickerToolBar = [[UIToolbar alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, ToolBarHeight)];
        pickerToolBar.items = barItems;
        [pickView addSubview:pickerToolBar];

        //获取当前日期，储存当前时间位置
        NSArray *indexArray = [self getNowDate:self.scrollToDate];
        
        if (!myPickerView) {
            myPickerView = [[UIPickerView alloc]initWithFrame:CGRectMake(0, ToolBarHeight, self.frame.size.width, PickViewHeight)];
            myPickerView.showsSelectionIndicator = YES;
            myPickerView.backgroundColor = [UIColor clearColor];
            myPickerView.delegate = self;
            myPickerView.dataSource = self;
            [pickView addSubview:myPickerView];
        }
        //调整为现在的时间
...
- (void)dismiss
{
    [UIView animateWithDuration:AnimationDuration animations:^{
        overButton.backgroundColor = [UIColor clearColor];
        pickView.frame = CGRectMake(0, self.frame.size.height, self.frame.size.width, PickViewHeight);
...
-(void)show{
    [UIView animateWithDuration:AnimationDuration animations:^{
        overButton.backgroundColor = [UIColor colorWithRed:.16 green:.17 blue:.21 alpha:.5];
        pickView.frame = CGRectMake(0, self.frame.size.height-PickViewHeight, self.frame.size.width, PickViewHeight);
    }];
}
...
//            return self.frame.size.width/5-0;
            if (component==0) return self.frame.size.width/5+10;
            if (component==1) return self.frame.size.width/5-10;
            if (component==2) return self.frame.size.width/5-10;
            if (component==3) return self.frame.size.width/5-10;
            if (component==4) return self.frame.size.width/5-0;
        }
            break;
...
//                [pickerView reloadComponent:1];
    ...
    NSString *yearStr = [yearArray[yearIndex] substringWithRange:NSMakeRange(0, 4)];
    NSString *monthStr = [monthArray[monthIndex] substringWithRange:NSMakeRange(0, 2)];
    NSString *dayStr = [dayArray[dayIndex] substringWithRange:NSMakeRange(0, 2)];
    NSString *hourStr = [hourArray[hourIndex] substringWithRange:NSMakeRange(0, 2)];
    NSString *minStr = [minuteArray[minuteIndex] substringWithRange:NSMakeRange(0, 2)];
    NSDate *date = [self dateFromString:[NSString stringWithFormat:@"%@%@%@%@%@",yearStr,monthStr,dayStr,hourStr,minStr] withFormat:@"yyyyMMddHHmm"];
